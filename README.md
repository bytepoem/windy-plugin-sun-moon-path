...
- **云层距离是几何参考。** 当前点云高不代表远方云区；不沿光路采样天气、地形、云厚或消光，不保证目标可见或出现朝晚霞。调整时间时保持升落按钮所选的天体，按钮右侧切换云图，预报时次与时间输入、滑条同排。卫星云图为独立实况对照，不与未来预报时刻同步。云底离地高度与计算海拔分别标注。详见[云层计算说明](docs/cloud-obstruction.md)。
- **云高来源可切换。** 单层默认使用 Windy 预报云底，分层默认使用云量剖面（云量 ≥ 10%）；温湿剖面默认按温度－露点 ≤ 2℃ 估算候选云层，阈值可调，不等同实测，也不作冰面饱和修正。单层剖面取最低检出层；分层预报云底按模型地形以上高度归入低（< 2 km）、中（2～6 km）、高（≥ 6 km）中的一层，其他层未知。缺测不自动换方法，手动值和两种剖面阈值分别保留。
